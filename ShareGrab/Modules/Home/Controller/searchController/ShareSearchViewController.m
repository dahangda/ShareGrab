...
- (void)viewDidLoad {
    ...
    // 创建UISearchBar对象
    _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth - 20, 35)];
    UIBarButtonItem * searchButton = [[UIBarButtonItem alloc]initWithCustomView:_searchBar];
    self.navigationItem.leftBarButtonItem = searchButton;
    [_searchBar becomeFirstResponder];
    //     设置搜索背景色CGColor
    _searchBar.barTintColor = YHWhiteColor;
    ...
    // 创建UITableView
    atableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight - 64) style:UITableViewStylePlain];
    // 设置代理
    ...
    [labelRight mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(labelLeft.right).offset(5);
        make.bottom.mas_equalTo(labelLeft.bottom);
    }];
    UIImageView *lineView = [[UIImageView alloc]init];
    lineView.backgroundColor = YHRGBColor(245, 245, 245);
